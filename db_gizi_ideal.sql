...
ALTER TABLE `riwayat_diagnosa` 
ADD COLUMN `berat_badan` DECIMAL(5,2) NULL AFTER `nama_lengkap`,
ADD COLUMN `tinggi_badan` DECIMAL(5,2) NULL AFTER `berat_badan`,
ADD COLUMN `imt` DECIMAL(5,2) NULL AFTER `tinggi_badan`,
ADD COLUMN `data_gejala` TEXT NULL COMMENT 'JSON array gejala yang terdeteksi' AFTER `imt`;
